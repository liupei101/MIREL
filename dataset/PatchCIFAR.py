"""Pytorch Dataset class that loads perfectly balanced CIFAR dataset in bag form.
...
class CifarBags(data_utils.Dataset):
    def __init__(self, target_number=9, mean_bag_length=10, var_bag_length=1, mean_pos_ratio=0.5, num_bag=1000, seed=7, 
        ...
        # OOD settings
        all_classes = [i for i in range(len(self.classes))]
        ...
        if ood_origin is None:
            self.ood_origin = 'svhn'
        else:
            self.ood_origin = ood_origin
        assert self.ood_origin in ['svhn', 'dtd'] # SVHN, Texture
        ...
        print("[PatchCIFAR] {}: num_bag={}, seed={}, data_name={}, target_number={}, low_pos_ratio={}, ID labels={}, ood_origin={}, ood_ins_ratio={}, ood_num_bag={}.".\
            format(self.data_name, num_bag, seed, data_name, self.target_number, self.low_pos_ratio, self.label_ID, self.ood_origin, ood_ins_ratio, self.ood_num_bag))
        
    def _get_ID_data(self):
        if self.data_name in ['train', 'val']:
            # train and val will use different seeds to form bags
            cur_dataset = datasets.CIFAR10('./datasets', 
        ...
        images = cur_dataset.data
        labels = torch.LongTensor(cur_dataset.targets)
    ...
    def _get_OOD_data(self):
        if self.ood_origin == 'dtd':
            dtd_test = DTD('./datasets',
                split="test",
                download=True,
            )
            tf_resize = transforms.Compose([transforms.Resize((32, 32))])
            ood_images = [tf_resize(default_loader(f)) for f in dtd_test._image_files]
            ood_images = np.concatenate([np.expand_dims(np.array(img), 0) for img in ood_images], axis=0) # [N, 32, 32, 3]
            ood_labels = torch.LongTensor(dtd_test._labels)
        elif self.ood_origin == 'svhn':
            svhn_test = datasets.SVHN(
                './datasets',
                split='test',
                download=True,
            )
            ood_images = np.transpose(svhn_test.data, (0, 2, 3, 1)) # [N, 3, 32, 32] -> [N, 32, 32, 3]
            ood_labels = torch.LongTensor(svhn_test.labels)
        else:
            pass
    ...
    def __getitem__(self, index):
        ...
        for i in range(len(bag)):
            ...
            img = self.img_transform(img) # [3, 32, 32] and values are in [0, 1]
            tensor_bag.append(img.unsqueeze(0))

        tensor_bag = torch.cat(tensor_bag, dim=0) # [N, 3, 32, 32]
