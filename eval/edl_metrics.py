"""Metrics for evaluating UE tasks

Codes are adapted from https://github.com/danruod/IEDL/blob/main/code_classical/utils/metrics.py.
"""
import torch
import numpy as np
...
def compute_confidence_score_categorical(prob_output, uncertainty_type='max_prob'):
    # categorical_output (probs) from standard classification models: [B, num_cls] 
    pred = prob_output.clone()
    num_cls = pred.shape[-1]
    pred = pred.reshape(-1, num_cls) # [B, num_cls]
    if uncertainty_type == 'max_alpha':
        scores = pred.max(-1)[0].cpu().detach().numpy()
    elif uncertainty_type == 'max_prob':
        scores = pred.max(-1)[0].cpu().detach().numpy()
    elif uncertainty_type == 'alpha0':
        scores = None
    elif uncertainty_type == 'differential_entropy':
        entropy = Categorical(pred).entropy()
        scores  = - entropy.squeeze().cpu().detach().numpy()
    elif uncertainty_type == 'expected_entropy':
        entropy = Categorical(pred).entropy()
        scores  = - entropy.squeeze().cpu().detach().numpy()
...
def compute_confidence_score(alpha, uncertainty_type='max_prob'):
    if uncertainty_type == 'max_alpha':
    ...
    elif uncertainty_type == 'differential_entropy': # distributional uncertainty
        eps = 1e-6
        alpha = alpha + eps
        alpha0 = alpha.sum(-1)
    ...
    elif uncertainty_type == 'expected_entropy': # data uncertainty
        eps = 1e-6
        alpha = alpha + eps
        alpha0 = alpha.sum(-1)
        ...
        scores = - exp_data_uncertainty.cpu().detach().numpy()
    elif uncertainty_type == 'mutual_information': # distributional uncertainty
        eps = 1e-6
        alpha = alpha + eps
        alpha0 = alpha.sum(-1)
...
def confidence(Y, alpha, uncertainty_type='max_prob', save_path=None, return_scores=False):
    ...
    scores = compute_confidence_score(alpha, uncertainty_type)
