...
from .deepmil import DeepMIL, DSMIL
from .evmil import ABMIL


################################################
#    Functions for loading available models 
################################################
def load_model(task:str, backbone:str, dims:List, **kws):
    if task == 'clf':
        if backbone == 'DeepMIL':
            return Deep_MIL(dims, **kws)
        if backbone == 'ABMIL':
